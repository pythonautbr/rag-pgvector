...
class VectorStore:
    ...
    def __init__(self):
        ...
        self.pg_vector_store = VectorStorePG(self.settings.database.service_url)
    ...
    def insert_pg(self, id: str, vector: List[float], metadata: Optional[dict] = None):
        """Insert a vector with optional metadata into the PostgreSQL store."""

        self.pg_vector_store.insert(id, vector, metadata)


    def search_pg(self, query_vector: List[float], k: int = 5) -> List[dict]:
        """Search for similar vectors using cosine similarity in the PostgreSQL store."""

        return self.pg_vector_store.search(query_vector, k)
...
class VectorStorePG:
    ...
    def insert(self, id: str, vector: List[float], metadata: Optional[dict] = None):
        """Insert a vector with optional metadata into the store."""
        ...
        with psycopg2.connect(self.connection_string) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    LIMIT %s
                    """,
                    (np.array(query_vector), np.array(query_vector), k)
                )
                return [
